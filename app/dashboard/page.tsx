  ...
  return (
    <section className="space-y-8">
      ...
      <div className="space-y-3">
        ...
        {trophies.length === 0 ? (
          <div className="text-sm text-gray-600">No trophies yet. Complete an exam to earn one!</div>
        ) : (
      ...
      <div className="space-y-3">
        ...
        {progress.length === 0 ? (
          <div className="text-sm text-gray-600">No progress yet. Start a lesson to see updates here.</div>
        ) : (
          <ul className="space-y-2">
            {progress.map((p) => {
              ...
              if (p.lesson) {
                label = `Lesson ${p.lesson.number}: ${p.lesson.title}`
                language = p.lesson.language.name
              } else if (p.exam) {
                label = `Exam: ${p.exam.lesson.title}`
                language = p.exam.lesson.language.name
              }
                ...
                <li key={p.id} className="card p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <div className="text-xs text-gray-600">{language} • {status} • {when}</div>
                    </div>
